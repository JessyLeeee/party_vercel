<template>
  <div class="p-4 max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold mb-4">党员故事</h1>
    <div v-for="quote in quotes" :key="quote.id" class="border-b py-4">
      ...
      <p class="text-sm mt-2">分享人：{{ quote.submitter }}</p>
      <p class="text-xs mt-1 text-gray-500">{{ quote.created_at }}</p>
      <button @click="like(quote.id)" class="mt-2 text-blue-500">👍 点赞 ({{ quote.likes }})</button>
    </div>
  </div>
</template>
